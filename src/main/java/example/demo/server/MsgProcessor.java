...
public class MsgProcessor {
    ...
    public void logout(Channel client) {
        //如果nickName为null，没有遵从聊天协议的连接，表示为非法登录
    ...
    public void sendMsg(Channel client, String msg) {
        ...
        if (request.getCmd().equals(IMP.LOGIN.getName())) {
            ...
            for (Channel channel : onlineUsers) {
                ...
                if ("Console".equals(channel.attr(FROM).get()) & !isslef) {
                    channel.writeAndFlush(request);
                    continue;
                }
                String content = encoder.encode(request);
            ...
            if (null != attrs) {
                ...
                if (sub < 1000 * seconds) {
                    ...
                    request.setContent("您送鲜花太频繁了," + (seconds - Math.round(sub / 1000)) + "秒后再试");
                    String content = encoder.encode(request);
